...
为保证下游分析输入数据的可靠性，需要对下机原始测序数据进行质控。目前，测序公司通常会提供clean data，一般情况无需额外质控。

## 三 比对参考基因组

### **BWA**

## 三 GATK calling pipeline
...
glnexus (GL, genotype likelihood)用于对大规模gvcf 进行 `joint call`，其由DNAnexus公司开发。官方宣称glnexus可以代替 `GATK CombineGVCFs` 和 `GATK GenotypeGVCFs`
